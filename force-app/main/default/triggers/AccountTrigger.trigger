trigger AccountTrigger on Account (after update,before update) {
    ...
    if(trigger.isBefore && trigger.isUpdate){
        AccountOwnerUpdateYA.accountUpdate(trigger.new,trigger.old);
    }
   ...
   /* 
    if(trigger.isAfter && trigger.isUpdate){
        ...
        for(Account ac:trigger.new){
            if(ac.Detail__c!=NULL){
                setId.add(ac.id);
            }
        }
        
        List<Contact> contactList=new List<Contact>([select id,accountid,Email from contact where accountid in: setId]);
        map<Id,Account> accountMap=new map<Id,Account>([select id,Detail__c from Account where id in: setId]);
